...
NewLife.MySql 是新生命团队（NewLife）出品的**纯国产高性能 MySQL 驱动**，基于 ADO.NET 标准接口实现，直接通过 TCP 通信实现 MySQL 协议层（Protocol Version 10），完全不依赖 NewLife 团队以外的任何第三方组件。

**设计目标**：轻量级高性能，只支持 ADO.NET 标准的增删改查、事务、参数化查询、存储过程和批量操作，无需支持 EF、集群或设计器等功能。聚焦**大数据批量操作**场景，提供管道化执行、数组绑定等独有能力。
...
NewLife.MySql 不仅支持标准 MySQL，还原生支持 MySQL 协议兼容的分布式数据库：

| 数据库 | 支持状态 | 自动检测 | 说明 |
|--------|---------|---------|------|
| **MySQL** | ✅ 完整支持 | ✅ | 5.5 ~ 9.0 全版本 |
| **OceanBase** | ✅ 完整支持 | ✅ | 基于版本字符串自动识别 |
| **TiDB** | ✅ 完整支持 | ✅ | 基于版本字符串自动识别 |
| **MariaDB** | ✅ 基础支持 | ❌ | 缺少 ed25519 认证支持 |

**版本字符串检测规则**：
- OceanBase：`5.7.25-OceanBase-v4.0.0` → `DatabaseType.OceanBase`
- TiDB：`5.7.25-TiDB-v6.5.2` → `DatabaseType.TiDB`
- MySQL：`8.0.26` → `DatabaseType.MySQL`

**使用方式**：
```csharp
using var conn = new MySqlConnection(connStr);
conn.Open();
Console.WriteLine($"数据库类型: {conn.DatabaseType}");  // MySQL/OceanBase/TiDB
```
...
基于 .NET 10 + MySQL 8.0.26 本机压测，管道化 + 事务（tx+pipe）vs 逐行 SingleRow 基线：

| 场景 | 逐行基线 | 管道化+事务 | 加速比 |
|------|------:|------:|------:|
| 1,000 行 INSERT | 437ms | 54ms | **8.1x** |
| 10,000 行 INSERT | 3,150ms | 680ms | **4.6x** |
| 1,000 行 UPDATE | 347ms | 75ms | **4.6x** |
| 10,000 行 UPDATE | 3,469ms | 757ms | **4.6x** |
| 1,000 行 DELETE | 332ms | 79ms | **4.2x** |
| 10,000 行 DELETE | 3,351ms | 673ms | **5.0x** |

三驱动对比（10,000 行批量操作）：

| 操作 | NewLife Pipeline(tx) | MySql.Data Batch(tx) | MySqlConnector Batch(tx) | NewLife 加速比 |
|------|------:|------:|------:|------:|
| INSERT | 899ms | 1,927ms | 1,906ms | **2.1x** |
| UPDATE | 710ms | 2,265ms | 2,041ms | **2.9x** |
| DELETE | 661ms | 1,961ms | 1,767ms | **2.7x** |

> 管道化的收益主要来自减少网络往返延迟，延迟越高收益越大。本机内网测试已有 **4x ~ 8x** 加速，跨机房场景收益更高。
> 批量写入场景全面领先竞品 **2x~3x**。
> 
...
|---------|:------------:|:--------------:|:-------------------:|
| **架构层次** | 4 层（应用/ADO.NET/连接池/协议+网络） | 5 层（含中间件层） | 4 层 |
| **协议实现** | 单一 `SqlClient` 类 | `MySqlSession` + `MySqlProtocolSerializer` 分离 | `NativeDriver` 单体 |
| **IO 模型** | 纯 `async/await` | 纯 `async/await` + `ValueTask` | **同步为主**，异步为包装层 |
| **内存管理** | `ArrayPool` + `OwnerPacket` | `ArrayPool` + `SequenceReader` | 传统 `byte[]` 分配 |
| **连接池** | `ObjectPool<SqlClient>` 轻量实现 | 自定义 `ConnectionPool`，支持多策略 | 自定义连接池 |
| **代码行数** | ~3000 行 | ~30000 行 | ~50000 行 |
| **编程范式** | 面向过程+对象 | 面向对象+模式驱动 | 传统 OOP |
...
**特点**：
- **极简设计**：所有协议操作集中在 `SqlClient` 一个类中，约 900 行代码
- **包构建与发送分离**：`BuildExecutePacket` 独立构建包，方便管道化批量复用
- **双模式执行**：同一个 `ExecuteStatementPipelineAsync` 入口根据 `Pipeline` 设置自动选择管道化或串行

#### MySqlConnector 协议架构

```
MySqlSession（会话管理）
├── MySqlProtocolSerializer（协议序列化/反序列化）
├── MySqlConnectionStringBuilder（大量配置项）
├── ServerSession（服务端会话状态）
├── CachedProcedure（存储过程缓存）
└── 多个 Result 类型层次
```

**特点**：
- **职责分离**：协议序列化、会话管理、结果处理分为独立类
- **更丰富的功能**：支持压缩协议、流式大数据、多种认证插件
- **更高的复杂度**：代码量约为 NewLife.MySql 的 10 倍

#### MySql.Data (Oracle) 协议架构
...
├── MySqlStream（流封装，含压缩）
├── Authentication（认证）
└── ResultSet / Field（结果处理）
```

**特点**：
- **传统单体设计**：`NativeDriver` 承担大部分协议工作
- **同步 IO 为主**：异步方法多为 `Task.Run` 包装同步操作
- **GPL 许可**：商用需要购买商业许可

### 16.3 连接池架构对比

| 特性 | NewLife.MySql | MySqlConnector | MySql.Data |
|------|:------------:|:--------------:|:----------:|
| **基础实现** | 继承 `ObjectPool<T>` | 自定义 `ConnectionPool` | 自定义实现 |
| **分池策略** | 连接字符串哈希 | 连接字符串哈希 | 连接字符串哈希 |
| **健康检查** | `Reset()` + `PingAsync()` | `ResetConnectionAsync()` | `Ping()` |
| **服务器变量缓存** | ✅ 10 分钟 TTL | ❌ | ❌ |
| **最小/最大连接** | 10 / 100000 | 0 / 100 | 0 / 100 |
| **空闲回收** | 30 秒 / 全空闲 300 秒 | 可配置 | 可配置 |
| **连接验证时机** | 获取时验证 | 获取时验证 | 获取时验证 |
| **连接状态污染处理** | `Database` 比对，污染则销毁 | `ResetConnection` 命令 | `ResetConnection` 命令 |

### 16.4 批量操作架构对比

这是 NewLife.MySql 的**核心竞争力**所在：

| 批量方案 | NewLife.MySql | MySqlConnector | MySql.Data |
|---------|:------------:|:--------------:|:----------:|
| **预编译 + 循环执行** | ✅ 自动 Prepare | ✅ 手动 Prepare | ✅ 手动 Prepare |
| **字典参数集批量** | ✅ `ExecuteBatch` | ❌ | ❌ |
| **数组绑定批量** | ✅ `ExecuteArrayBatch` | ❌ | ❌ |
| **管道化执行** | ✅ `Pipeline=true` | ❌ | ❌ |
| **多行 VALUES** | ✅ 文本拼接 | ✅ `MySqlBulkCopy` | ✅ 文本拼接 |
| **DbBatch** | ✅ (.NET 6+) | ✅ (.NET 6+) | ❌ |
| **BulkCopy** | ❌ | ✅ `MySqlBulkCopy` | ❌ |

#### 管道化执行的独创性

NewLife.MySql 的管道化执行是**其他 MySQL 驱动所不具备的能力**：

```
传统驱动（串行模式）：
  Send(1) → Recv(1) → Send(2) → Recv(2) → ... → Send(N) → Recv(N)
  耗时 = N × (网络延迟 × 2 + 处理时间)

NewLife.MySql 管道化：
  Send(1) → Send(2) → ... → Send(N) → Flush → Recv(1) → Recv(2) → ... → Recv(N)
  耗时 = 批量发送时间 + N × 处理时间 + 批量接收时间 ≈ 1 × 网络延迟 × 2 + N × 处理时间
```

**关键设计决策**：
- `BuildExecutePacket` 与 `SendPacketAsync` 解耦，支持先构建后批发
- `SendPacketAsync` 的 `flush` 参数控制是否立即刷新网络流
- 每个 `COM_STMT_EXECUTE` 包独立使用 `seq=0`，互不干扰
- 错误处理：管道中某个命令失败，继续读取后续响应保持连接干净

### 16.5 IO 模型对比

| 特性 | NewLife.MySql | MySqlConnector | MySql.Data |
|------|:------------:|:--------------:|:----------:|
| **异步模型** | 纯 `Task` | `ValueTask` | `Task`（包装同步） |
| **超时机制** | `CancellationTokenSource.CancelAfter` | `CancellationTokenSource` | `Socket.ReceiveTimeout` |
| **包读取** | `ReadExactlyAsync` + `ArrayPool` | `SequenceReader` + `PipeReader` | `MySqlStream.Read` |
| **包发送** | `SendPacketAsync` + 延迟 Flush | 直接发送 | 直接发送 |
| **SSL 升级** | `SslStream` + 条件编译 TLS 版本 | `SslStream` | `SslStream` |
| **网络残留清理** | `Reset()` 主动读取 | `ResetConnectionAsync` 命令 | 不主动清理 |

### 16.6 认证架构对比

| 特性 | NewLife.MySql | MySqlConnector | MySql.Data |
|------|:------------:|:--------------:|:----------:|
| **认证插件架构** | 内置 `Authentication` 类 | 插件系统 `IAuthenticationPlugin` | 内置 + 可扩展 |
| **mysql_native_password** | ✅ SHA1 双重哈希 | ✅ | ✅ |
| **caching_sha2_password** | ✅ SHA256 + RSA 全量认证 | ✅ | ✅ |
| **Auth Switch** | ✅ 0xFE 切换 | ✅ | ✅ |
| **RSA 公钥传输** | ✅ 请求服务器公钥 | ✅ 支持本地缓存 | ✅ |
| **连接属性发送** | ✅ `CONNECT_ATTRS` | ✅ | ✅ |
| **MFA 多因子标志** | ✅ 协商标志 | ✅ | ✅ |
...
| **需要 MariaDB 专属特性** | MySqlConnector | ed25519 认证等 |
| **遗留 .NET Framework 项目** | **NewLife.MySql** / MySql.Data | 均支持 net45+ |

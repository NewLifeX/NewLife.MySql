...
| MinPoolSize | Min Pool Size, Minimum Pool Size | 0 | 连接池最小连接数 |
| MaxPoolSize | Max Pool Size, Maximum Pool Size | 100 | 连接池最大连接数 |
| ConnectionLifeTime | Connection Lifetime, Connection Life Time | 600 | 连接最大存活期（秒），到龄主动回收，0 禁用 |
| ConnectionIdlePingTime | Connection Idle Ping Time, Idle Ping Time | 30 | 空闲多久借出前需 PING 验活（秒），兜底黑洞型断连，0 禁用 |
...
| **Apache Doris** | ✅ 完整支持 | 版本含 `Doris`（如 `5.7.99 Doris version doris-2.0.3`） | 基于版本字符串自动识别 |
...
- 校验失败的连接走基类 discard-retry 生命周期，`BusyCount` 记账无泄漏

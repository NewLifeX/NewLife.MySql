...
| **最新版本** | v1.3.2026.0802 | v2.6.1 | v9.7.0 |
...
|------|------:|------:|------:|:------:|:------:|
| SingleRow（逐行） | 1172.41 | 1342.71 | **1141.92** | 1.15x 快 | — |
| BulkRead（批量读取） | **6.07** | 7.89 | 6.75 | **1.30x** | 1.11x |
| DbTable（列式填充） | **6.07** | 20.41 | 6.82 | **3.36x** | 1.12x |
| ReadModels（实体映射） | **6.91** | 25.21 | 10.11 | **3.65x** | **1.46x** |

**结论**：批量 SELECT 场景 NL 小幅领先 Connector（11~46%），远超 Official（30~265%）。NL 在 ReadModels 场景差距最大——原生路径直接从网络字节流映射实体，跳过了 DbTable 中间层，每行映射开销仅 **~84ns**（Connector ~336ns/行，Official ~1,732ns/行）。
...
|------|------:|------:|------:|:------:|:------:|
| INSERT | **79.30** | 117.07 | 113.67 | **1.48x** | **1.43x** |
| UPDATE | **79.50** | 126.51 | 119.95 | **1.59x** | **1.51x** |
| DELETE | **82.42** | 120.92 | 116.00 | **1.47x** | **1.41x** |

**结论**：NL Pipeline(tx) 在批量 DML 上稳定领先竞品 **41~59%**。优势来自管道化（批量发送+批量接收，单次网络往返）+ 事务（N 次 fsync 合并为 1 次）+ 数组绑定（零装箱参数传递）三重加速。

### 5.3 性能特征总结

| 场景 | 推荐驱动 | 理由 |
|------|---------|------|
| 大数据批量 DML | **NewLife.MySql** | Pipeline + 事务，37~57% 领先 |
| 大批量 SELECT + 实体映射 | **NewLife.MySql** | ReadModels 原生路径，每行 73ns |
| 超大批量导入（百万级） | NewLife.MySql / MySqlConnector | NL 有 BulkCopy + Pipeline 双引擎 |
| 逐行散点查询 | MySqlConnector | 微弱领先（+5.5%），NL 有意取舍 |
| 跨网络低带宽场景 | **NewLife.MySql** | Pipeline + 压缩协议双重优化 |

...
| **NuGet 总下载** | 5,800 | 2.397 亿 | 1.444 亿 |
| **NuGet 日均下载** | ~45 | ~223,000 | ~84,000 |
...
| ASP.NET Core DI 集成 | 中 | 低 | P1 |
| 压缩协议支持 | 低 | 中 | P2 |
| Geometry 空间数据类型 | 低 | 中 | P2 |
| Unix Socket 连接 | 低 | 低 | P2 |
| MySqlBulkCopy | 低 | 高 | P3（Pipeline 可替代） |
| 英文文档 | 中 | 中 | P1 |
...
| 竞品功能 | 竞品 | 是否值得跟进 | 理由 |
|------|------|:---:|------|
| MySqlBulkCopy（LOAD DATA） | MySqlConnector | ❌ | Pipeline 在万级行场景已足够；LOAD DATA 需文件权限，部署受限 |
...
| P1 | 实现 ASP.NET Core DI 集成（`AddMySqlDataSource`） | 吸引 ASP.NET Core 用户 |
| P1 | 编写迁移指南（MySql.Data → NL、MySqlConnector → NL） | 降低迁移摩擦 |
| P1 | 评估 NativeAOT 兼容性（net8.0+） | 跟进 .NET 现代化趋势 |
| P2 | 补充压缩协议 | 对跨网络场景有明显收益 |
| P2 | 补充 Geometry 类型支持 | 拓展 GIS 领域用户 |

...
| 驱动 | 100 行 | 1,000 行 | 10,000 行 |
|------|------:|------:|------:|
| NewLife SingleRow | 15.04 | 131.66 | 1,228.45 |
| Official SingleRow | 20.80 | 155.98 | 1,149.99 |
| Connector SingleRow | 13.31 | 112.46 | 887.35 |
...
| 驱动 | 100 行 | 1,000 行 | 10,000 行 |
|------|------:|------:|------:|
| NewLife BulkRead | 1.45 | 4.70 | 24.84 |
| Official BulkRead | 1.47 | 3.56 | 14.04 |
| Connector BulkRead | 0.80 | 1.44 | **6.46** |

#### BulkRead 加速比（vs 同驱动 SingleRow）

| 驱动 | 100 行 | 1,000 行 | 10,000 行 |
|------|------:|------:|------:|
| NewLife BulkRead | **10.4x** | **28.0x** | **49.4x** |
| Official BulkRead | 14.1x | 43.8x | 81.9x |
| Connector BulkRead | 16.6x | 78.1x | 137.4x |

#### 三驱动 BulkRead 对比（vs NewLife BulkRead 基线）

| 驱动 | 100 行 | 1,000 行 | 10,000 行 |
|------|------:|------:|------:|
| NewLife BulkRead | 1.0x | 1.0x | 1.0x |
| Official BulkRead | 1.0x | 1.3x | 1.8x |
| Connector BulkRead | 1.8x | 3.3x | **3.8x** |

#### SELECT 分析

1. **BulkRead vs SingleRow 是量级差异**：NewLife BulkRead 在 10,000 行仅 24.84ms（**49.4x**），根本原因是消除了 N-1 次额外网络往返——N 个单行查询需要 N 次 Round Trip，BulkRead 只需 1 次。

2. **`_Values[]` 复用优化已生效**：BulkRead 场景中，同一个 `reader` 连续调用 `reader.Read()` N 次，`_Values[]` 数组从第 2 行起全部复用，彻底消除了每行 `new Object[FieldCount]` 分配。SingleRow 场景每次创建新 reader，无法受益于此优化——这也是两种模式必须分开测量的原因。

3. **NewLife BulkRead 与 Connector 存在差距（3.8x）**：Connector 10,000 行 BulkRead 仅 6.46ms，NewLife 为 24.84ms。差距来源主要是结果集解析路径的值类型装箱——当前 `_Values[]` 仍是 `Object[]`，所有 `Int32`/`Double`/`DateTime` 在 `ReadTextValue` 阶段均需装箱为 `Object`。消除装箱（改为按列类型分区存储）是 SELECT 性能的下一个重要优化方向。

4. **后续优化方向**：
   - 结构化列存储（`Int64[]`/`Double[]`/`String[]` 分列而非 `Object[]`），彻底消除装箱
   - 在 `GetXxx(ordinal)` 中直接从原生类型数组读取，绕过 `Convert.ToXxx()` 反射路径
...
| 批量 DELETE 性能 | ★★★★★ | ★★★ | ★★★ |
| SELECT 逐行查询 | ★★★★ | ★★★ | ★★★★ |
| SELECT 批量读取（BulkRead） | ★★★ | ★★★★ | ★★★★★ |
| 管道化支持 | ✅ 独创 | ❌ | ❌ |
| 数组绑定 API | ✅ | ❌ | ❌ |

> NewLife.MySql 在批量写入（INSERT/UPDATE/DELETE）凭借管道化+事务组合全面领先竞品 **2x~3x**；SELECT BulkRead 场景目前落后 Connector 约 3.8x，主要瓶颈为结果集解析时的值类型装箱，是下一阶段重点优化方向。

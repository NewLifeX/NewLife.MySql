...
**NewLife.MySql** 是新生命团队（NewLife）出品的**纯国产高性能 MySQL 驱动**，基于 ADO.NET 标准接口实现，直接通过 TCP 通信实现 MySQL 协议层（Protocol Version 10），完全不依赖 NewLife 团队以外的任何第三方组件，采用 **MIT 开源协议**。

仅依赖 `NewLife.Core`（同为新生命团队出品），实现了一个**零第三方依赖**、**全链路真异步**、**支持大数据批量操作**的纯托管 MySQL 客户端。
...
> 本机内网延迟极低时已有 **4× ~ 8×** 加速，跨机房高延迟场景收益更大。详细数据参见 [性能测试报告](Doc/性能测试报告.md)。
...
- **零额外分配**：使用 `ArrayPool<T>` 和 `OwnerPacket` 管理内存，减少 GC 压力
- **对象池复用**：`Pool.StringBuilder` / `Pool.MemoryStream` / `Pool.Shared` 全面池化
...
- **MySQL 版本**：5.5 ~ 9.0+，支持 `mysql_native_password` 和 `caching_sha2_password`
...
| ed25519 (MariaDB) | ❌ | ✅ | ❌ |
| DbDataSource (.NET 7+) | ❌ | ✅ | ❌ |
| EF Core 支持 | ✅ NewLife.MySql.EntityFrameworkCore | ✅ (via Pomelo) | ✅ |
| 压缩协议 | ❌ | ✅ | ✅ |
| 负载均衡/故障转移 | ❌ | ✅ | ✅ |
| LOAD DATA LOCAL | ❌ | ✅ | ✅ |
...
`NewLife.MySql.EntityFrameworkCore` 包提供了基于 NewLife.MySql 驱动的 EF Core 数据库提供程序，支持 .NET 6 / .NET 8 / .NET 10。
...
| 功能 | 说明 |
|------|------|
| 类型映射 | Int / Long / String / DateTime / Decimal / Guid / Boolean / Byte[] 等常用类型 |
| 查询翻译 | LINQ 查询生成 MySQL 语法的 SQL（反引号转义、LIMIT/OFFSET） |
| 方法翻译 | String.Contains/StartsWith/EndsWith/ToUpper/ToLower/Trim/Replace/Substring/Length |
| 成员翻译 | DateTime.Year/Month/Day/Hour/Minute/Second |
| 迁移支持 | CreateTable/AlterColumn/AddColumn/DropColumn/RenameTable/CreateIndex/DropIndex 等 |
| 值生成策略 | AUTO_INCREMENT 自增主键 |
| DbConnection 重载 | 支持传入已有的 DbConnection 实例 |
...
- **连接认证**：支持 `mysql_native_password` 和 `caching_sha2_password`
...
- [架构设计](Doc/架构设计.md) — 核心功能的架构设计与协议实现细节
- [性能测试报告](Doc/性能测试报告.md) — 批量操作基准性能测试报告
